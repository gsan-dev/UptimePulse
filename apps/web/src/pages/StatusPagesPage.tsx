  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-8">
      ...
      <form onSubmit={(e) => void handleSubmit(e)} className="mb-8 space-y-4 rounded-xl border border-white/10 bg-white/5 p-6">
        ...
        <Field label="Título">
          <input required value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Estado de Mi Empresa" className={inputClass} />
        </Field>
          ...
          <div className="space-y-1 rounded-md border border-white/10 bg-black/20 p-2">
            {monitors.length === 0 ? (
              ...
              monitors.map((monitor) => (
                <label key={monitor.id} className="flex items-center gap-2 px-1 py-1 text-sm text-gray-200">
                  <input
                    type="checkbox"
      ...
      <ul className="space-y-2">
        {pages?.map((page) => (
          <li key={page.id} className="flex items-center justify-between rounded-lg border border-white/10 bg-white/5 px-4 py-3">
            <div>
